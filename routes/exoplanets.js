...
router.post('/add', upload.single('imageExoplanet'), function (req, res, next) {
  ...
  if (
    validator.isLength(req.body.uniqueNameExoplanet, { min: 3, max: 100 })
  ) {
    ...
    res.redirect(
      "/exoplanets?errors= Le nom d'une exoplanète doit faire entre 3 et 100 caractères"
    )
  }
})
...
router.get('/filter', function (req, res, next) {
  ...
  if (filter === 'Filtrer par hclass') {
    ...
    exoplanetsTableFilter = Exoplanet.searchByHclass(
      req.query.hClassExoplanet
    )
  }
  ...
  res.render('exoplanets/index.hbs', {
    exoplanetsTable: exoplanetsTableFilter
  })
})
