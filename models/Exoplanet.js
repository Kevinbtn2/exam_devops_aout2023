...
module.exports.save = (data) => {
  ...
  if (data.id === undefined) {
    const stmt = db.prepare(
      'INSERT INTO EXOPLANETS(unique_name, hclass, discovery_year, image) VALUES (?, ?, ?, ?)'
    )
    const info = stmt.run(
      data.uniqueName,
      data.hClass,
  ...
  // id => update exoplanet
    ...
    const stmt = db.prepare(
      'UPDATE EXOPLANETS SET unique_name = ?, hclass = ?, discovery_year = ?, ist = ?, pclass = ? WHERE exoplanet_id = ?'
    )
    const info = stmt.run(
      data.uniqueName,
      data.hClass,
...
module.exports.search = (uniqueName) => {
  ...
  return db
    .prepare('SELECT * FROM EXOPLANETS WHERE unique_name LIKE ?')
    .all(uniqueName + '%')
}

module.exports.delete = (id) => {
  const info = db
    .prepare('DELETE FROM EXOPLANETS WHERE exoplanet_id = ?')
    .run(id)
...
module.exports.searchByYear = (year) => {
  return db
    .prepare('SELECT * FROM EXOPLANETS WHERE discovery_year = ?')
    .all(year)
}
